...
@section('content')
    ...
    <article class="box">
        ...
        <div class="box__content">
            <form action="{{ helium_route('legals.update') }}" method="post">
                {{ csrf_field() }}

                {!! Form::create('textarea', 'legals')->value($settings->get('legals'))->attr(['class' => 'f-100']) !!}
...
@section('js')
    ...
    <script>
        $('[name=legals]').froalaEditor({
            language: 'fr',
            toolbarButtons: ['paragraphFormat', 'bold', 'italic', 'underline', 'formatUL', 'align', '|',
                'insertLink', 'insertTable', 'insertImage', '|', 'outdent', 'indent', 'insertTable', '|',
                'fullscreen', 'undo', 'redo', 'clearFormatting'
            ],
            height: 300,
            heightMax: 500
        });
