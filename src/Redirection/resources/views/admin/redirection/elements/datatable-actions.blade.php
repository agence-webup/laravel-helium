  ...
  <div class="dropmic-menu" aria-hidden="true">
    <ul class="dropmic-menu__list" role="menu">
      ...
      <li class="dropmic-menu__listItem" role="menuitem">
        <a class="dropmic-menu__listContent"
          href="{{ route('admin.tools.redirection.edit', ['id' => $redirection->id]) }}" tabindex="-1">Editer</a>
      </li>

      <li class="dropmic-menu__listItem" role="menuitem">
        <form action="{{ route('admin.tools.redirection.destroy', $redirection->id) }}" method="post" class="inline">
          {{ method_field('delete') }}
          {{ csrf_field() }}
