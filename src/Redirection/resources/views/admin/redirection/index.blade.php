...
Helium::header()->add('Ajouter', helium_route_name('tools.redirection.create'));
Helium::header()->contextual([
    ...
    'Importer un csv' => [
        'route' => helium_route_name('tools.redirection.import'),
    ],
]);
@endphp
...
@section('content')
    ...
    <form id="deleteRedirections" action="{{ helium_route('tools.redirection.destroyAll') }}" method="post">
        {{ method_field('delete') }}
        {{ csrf_field() }}
    </form>
@endsection
...
@section('js')
    <script type="text/javascript">
        $('.dataTable').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{{ helium_route('tools.redirection.datatable') }}",
            columns: [{
                    data: 'from',
                    name: 'from'
                },
            ...
            rowCallback: function(row, data, index) {
                row.dataset.link = "{{ helium_route('tools.redirection.edit', ['id' => '%id%']) }}".replace(
                    "%id%", data.id);
            ...
            initComplete: function() {
                ...
                this.api().columns().every(function() {
                    ...
                    if (headerSearchable) {
                        ...
                        input.placeholder = "Search by " + column.header(column[0]).innerHTML
                            .toLowerCase();
