    ...
    <div class="dropmic-menu" aria-hidden="true">
        <ul class="dropmic-menu__list" role="menu">
            <li class="dropmic-menu__listItem" role="menuitem">
                <a class="dropmic-menu__listContent" href="{{ helium_route('contact.show', ['id' => $contact->id]) }}"
                    tabindex="-1"><i class="fa fa-eye"></i> Voir</a>
            </li>
            <li class="dropmic-menu__listItem" role="menuitem">
                <form action="{{ helium_route('contact.destroy', ['id' => $contact->id]) }}" method="post"
                    class="inline">
                    {{ method_field('DELETE') }}
                    {{ csrf_field() }}
